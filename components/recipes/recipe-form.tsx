...
export function RecipeForm() {
  ...
  }, [session, supabase, toast, router, recipeId])
    ...
    <div className="space-y-8 max-w-8xl mx-auto p-6">
      ...
      <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ImageUploader 
            onImageUpload={(url) => setFormData((prev: Partial<RecipeFormData>) => ({ ...prev, image: url }))} 
          />
          ...
          <div className="space-y-6">
            <RecipeBasicInfo
              ...
              onTitleChange={(value) => setFormData((prev: Partial<RecipeFormData>) => ({ ...prev, title: value }))}
              onDescriptionChange={(value) => setFormData((prev: Partial<RecipeFormData>) => ({ ...prev, description: value }))}
            />

            <RecipeMetadata
              cookingTime={formData.cooking_time || 0}
              temperature={formData.temperature || 0}
              onCookingTimeChange={(value) => setFormData((prev: Partial<RecipeFormData>) => ({ ...prev, cooking_time: value }))}
              onTemperatureChange={(value) => setFormData((prev: Partial<RecipeFormData>) => ({ ...prev, temperature: value }))}
            />
          </div>
        </div>
